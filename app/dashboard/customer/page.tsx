...
export default function CustomerDashboard() {
    ...
    const [reviewData, setReviewData] = useState({ productId: 0, orderId: 0, rating: 5, comment: '' });
    ...
    const fetchUserReviews = async () => {
        if (!user) return;
        const { data } = await supabase.from('reviews').select('product_id, order_id').eq('user_id', user.id);
        if (data) {
            setUserReviews(new Set((data as any[]).map(r => `${r.order_id}-${r.product_id}`)));
        }
    };

    const fetchWishlist = async () => {
        if (!user) {
            console.log('fetchWishlist: No user found');
            return;
        }

        console.log('fetchWishlist: Starting fetch for user', user.id);
        ...
        try {
            ...
            if (favError) {
                console.error('fetchWishlist: Error fetching favorites table:', favError);
                return;
            }

            console.log('fetchWishlist: Favorites raw data:', favData);

            if (!favData || favData.length === 0) {
                console.log('fetchWishlist: No favorites found in table.');
                setWishlist([]);
                return;
            }
            ...
            console.log('fetchWishlist: Fetching products for IDs:', productIds);
            ...
            if (prodError) {
                console.error('fetchWishlist: Error fetching products table:', JSON.stringify(prodError));
                return;
            }

            console.log('fetchWishlist: Products raw data:', productsData);

            // Step 3: Map and Merge
            if (productsData) {
                const productsMap = new Map(productsData.map(p => [p.id, p]));

                const joinedData = favData.map((fav: any) => {
                ...
                console.log('fetchWishlist: Final joined data:', joinedData);
                setWishlist(joinedData);
            }
        } catch (err) {
            console.error('fetchWishlist: Unexpected error:', err);
        }
    };
    ...
    // Effects
    useEffect(() => {
        if ((activeTab === 'orders' || activeTab === 'profile') && user) {
            fetchOrders();
        }
    ...
    const handleSubmitReview = async (e: React.FormEvent) => {
        ...
        if (error) {
            ...
            setReviewData({ productId: 0, orderId: 0, rating: 5, comment: '' });
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className={['orders', 'techcoin'].includes(activeTab) ? "col-span-1 md:col-span-4" : "col-span-1 md:col-span-3"}>
                        ...
                        {activeTab === 'wishlist' && (
                            <div className="space-y-6 animate-fade-in">
                                <h2 className="text-xl font-bold flex items-center gap-2 text-gray-800">
                                    <Heart className="text-[var(--primary-red)]" /> สินค้าที่ถูกใจ
                                </h2>
                                {wishlist.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {wishlist.map((item) => (
                                            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col relative group overflow-hidden hover:border-red-200 transition-colors">
                                                <Link href={`/product/${item.product.id}`} className="block">
                                                    <div className="aspect-square bg-gray-50 overflow-hidden relative">
                                                        <img src={item.product.image || item.product.image_url} alt={item.product.name} className="w-full h-full object-contain p-4 group-hover:scale-105 transition-transform" />
                                                    </div>
                                                </Link>
                                                <div className="p-4 flex flex-col flex-1">
                                                    <Link href={`/product/${item.product.id}`}>
                                                        <h3 className="font-bold text-gray-900 line-clamp-2 mb-2 hover:text-[var(--primary-red)] transition-colors text-sm h-10">{item.product.name}</h3>
                                                    </Link>
                                                    <div className="mt-auto flex justify-between items-center">
                                                        <p className="text-[var(--primary-orange)] font-bold text-lg">฿{item.product.price.toLocaleString()}</p>
                                                        <button
                                    ...
                                    <div className="bg-white p-12 rounded-xl shadow-sm flex flex-col items-center justify-center text-gray-400 border border-dashed border-gray-300">
                                        ...
                                        <p className="text-gray-500 font-medium">คุณยังไม่มีสินค้าที่ถูกใจ</p>
                                        <button onClick={() => router.push('/')} className="mt-6 px-6 py-2 bg-[var(--primary-red)] text-white rounded-lg font-bold hover:bg-red-700 transition-colors shadow-lg shadow-red-100">
                                            เลือกซื้อสินค้า
                                        </button>
                                    </div>
                                )}
                                                                        ...
                                                                        <div className="flex-1">
                                                                            ...
                                                                            <div className="flex items-center gap-2 mt-2">
                                                                                {order.status === 'completed' && (
                                                                                    !userReviews.has(`${order.id}-${item.product_id}`) ? (
                                                                                        <button onClick={() => openReviewModal(item.product_id, order.id)} className="ml-2 text-xs md:text-sm flex items-center gap-1.5 bg-yellow-400 hover:bg-yellow-500 text-white font-bold px-3 py-1.5 rounded-lg shadow-sm hover:shadow-md transition-all transform hover:-translate-y-0.5">
                                                                                            <Star className="w-3.5 h-3.5 fill-white" /> รีวิวสินค้า
                                                                                        </button>
                                                                                    ) : (
                                                                                        <button disabled className="ml-2 text-xs md:text-sm flex items-center gap-1.5 bg-gray-100 text-gray-400 font-bold px-3 py-1.5 rounded-lg cursor-not-allowed">
                                                                                            <Star className="w-3.5 h-3.5 fill-gray-400" /> รีวิวแล้ว
                                                                                        </button>
                                                                                    )
                                                                                )}
                                                                            </div>
